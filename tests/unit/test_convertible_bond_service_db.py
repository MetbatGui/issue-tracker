"""ConvertibleBondService의 DB SSOT 배선(parse_and_export_to_excel/export_to_excel/get_relation_map)
자체를 검증하는 테스트. (CapitalIncreaseService/BonusSharesService 쪽 배선 테스트와 동일한 이유로 작성)
"""
import shutil
...
def _make_decision(rcept_no: str, parent_rcp_no=None, company_name: str = "테스트회사") -> ConvertibleBondDecision:
    return ConvertibleBondDecision(
        ...
        shares_ratio=5.0,
        conversion_start_date=date(2025, 1, 1),
...
def service_with_real_xml_samples(tmp_path):
    """실제 전환사채 XML 샘플 2개를 임시 데이터 디렉토리에 복사해 파서 연동까지 실제로 태운다."""
