...
class CapitalIncreaseService(BaseReportService):
    ...
    def _parse_document_with_map(self, document: DownloadedXml, relation_map: dict) -> CapitalIncreaseDecision:
    ...
    def full_update(self, start_date: str = "20200101", end_date: str = None) -> LocalUpdateResult:
        ...
        self.logger.info("유상증자 데이터 전체 업데이트 시작")
    ...
    def daily_update(self, days_back: int = 1, today=None) -> LocalUpdateResult:
        ...
        result = self._result_after_collection(documents, relation_map)
        self.logger.info("유상증자 일일 업데이트 완료")
        return result
