"""전환사채 Excel -> SQLite 1회성 마이그레이션 스크립트

기존 data/전환사채/전환사채.xlsx(연도별 시트)를 읽어 ConvertibleBondSqliteRepository로 백필합니다.
...
def _row_to_decision(row: pd.Series) -> Optional[ConvertibleBondDecision]:
    ...
    return ConvertibleBondDecision(
        ...
        conversion_ratio=_cell_to_float(row.get("전환비율")),
        conversion_price=_cell_to_int(row.get("전환가액")),
        conversion_shares=_cell_to_int(row.get("전환에 따라 발행할 주식수")),
        shares_ratio=_cell_to_float(row.get("주식총수 대비 비율")),
        conversion_start_date=_cell_to_date(row.get("전환청구기간시작일")),
        conversion_end_date=_cell_to_date(row.get("전환청구기간종료일")),
...
def migrate(excel_path: str, db_path: str) -> dict:
    ...
    """
    repo = ConvertibleBondSqliteRepository(db_path)

    excel_file = pd.ExcelFile(excel_path)
    decisions = []

    for sheet_name in excel_file.sheet_names:
        # CB 엑셀은 CI/Bonus와 달리 startrow=0(BaseBondExcelWriter)이라 헤더가 0번째 행
    ...
    db_row_count = repo._conn.execute("SELECT COUNT(*) FROM convertible_bond_decisions").fetchone()[0]
...
if __name__ == "__main__":
    result = migrate("data/전환사채/전환사채.xlsx", "data/전환사채/전환사채.db")
