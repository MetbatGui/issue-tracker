"""전환사채 Excel -> SQLite 마이그레이션 스크립트 테스트

실제 ConvertibleBondExcelWriter로 생성한 Excel을 입력으로 사용해, 마이그레이션 후
...
from migrate_convertible_bond_to_sqlite import migrate


def _make_decision(rcept_no: str, year: int, company_name: str = "테스트회사") -> ConvertibleBondDecision:
    return ConvertibleBondDecision(
        source_filename=f"{company_name}_{rcept_no}.xml",
        company_name=company_name,
        sequence_number="1",
        bond_type="무기명식 무보증 사모 전환사채",
        ...
        conversion_shares=1_000_000,
        shares_ratio=5.0,
        conversion_start_date=date(year + 1, 1, 1),
        conversion_end_date=date(year + 3, 1, 1),
...
def sample_excel(tmp_path):
    excel_path = tmp_path / "전환사채.xlsx"
    writer = ConvertibleBondExcelWriter(output_path=str(excel_path))
    decisions = [
