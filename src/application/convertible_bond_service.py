...
class ConvertibleBondService(BaseReportService):
    ...
    def __init__(
        ...
        self.database_session = SqliteStorageSession.get_shared(self.source_storage, self.data_directory / "전환사채.db")
        self.repository = ConvertibleBondSqliteRepository(str(self.database_session.working_path))
    ...
    def _parse_document_with_map(self, document: DownloadedXml, relation_map: dict) -> ConvertibleBondDecision:
    ...
    def daily_update(self, days_back: int = 30, today=None) -> LocalUpdateResult:
