"""신주인수권부사채 SQLite 리포지토리 인프라스트럭처

신주인수권부사채 결정(Decision) 데이터를 SQLite에 SSOT로 저장/조회합니다.
"""
import sqlite3
...
__all__ = ["BondWithWarrantSqliteRepository"]


_SCHEMA = """
CREATE TABLE IF NOT EXISTS bond_with_warrant_decisions (
...
CREATE TABLE IF NOT EXISTS funding_purposes (
    decision_rcept_no TEXT NOT NULL REFERENCES bond_with_warrant_decisions(rcept_no),
...
_UPSERT_COLUMNS = [
    ...
    "exercise_ratio", "exercise_price", "exercise_shares", "shares_ratio",
    "exercise_start_date", "exercise_end_date", "subscription_date", "payment_date",
...
class BondWithWarrantSqliteRepository(ReportRepository):
    """신주인수권부사채 결정 데이터를 SQLite에 저장/조회하는 리포지토리."""
    ...
    def upsert(self, decisions: List[BondWithWarrantDecision]) -> int:
        """결정 목록을 rcept_no 기준으로 upsert합니다.

        Args:
            decisions: 저장할 신주인수권부사채 결정 객체 리스트
        ...
        sql = f"""
            INSERT INTO bond_with_warrant_decisions ({", ".join(_UPSERT_COLUMNS)})
            VALUES ({placeholders})
    ...
    def get_all(self) -> List[BondWithWarrantDecision]:
        """저장된 모든 신주인수권부사채 결정을 조회합니다."""
        rows = self._conn.execute(
            f"SELECT {', '.join(_UPSERT_COLUMNS)} FROM bond_with_warrant_decisions"
        ).fetchall()
    ...
    def _to_row_params(self, decision: BondWithWarrantDecision) -> dict:
        return {
            ...
            "exercise_ratio": decision.exercise_ratio,
            "exercise_price": decision.exercise_price,
            "exercise_shares": decision.exercise_shares,
            "shares_ratio": decision.shares_ratio,
            "exercise_start_date": _date_to_iso(decision.exercise_start_date),
            "exercise_end_date": _date_to_iso(decision.exercise_end_date),
    ...
    def _row_to_decision(self, row: tuple) -> BondWithWarrantDecision:
        ...
        return BondWithWarrantDecision(
            ...
            exercise_ratio=values["exercise_ratio"],
            exercise_price=values["exercise_price"],
            exercise_shares=values["exercise_shares"],
            shares_ratio=values["shares_ratio"],
            exercise_start_date=_iso_to_date(values["exercise_start_date"]),
            exercise_end_date=_iso_to_date(values["exercise_end_date"]),
